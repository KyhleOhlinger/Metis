...
// Compartment used to hot-swap just the background colour without rebuilding
// the entire editor state.
const bgCompartment = new Compartment();
...
function highlightForPreset(p: BgPreset) {
  return p.isDark ? metisHighlightStyleDark : metisHighlightStyleLight;
}

function plannerThemeVars(p: BgPreset): CSSProperties {
...
/** Builds a minimal EditorView theme that overrides only colour-related rules.
 *  Uses !important on background properties to reliably beat oneDark's base styles. */
function makeBgTheme(p: BgPreset) {
  return EditorView.theme(
    {
      "&": {
        backgroundColor: `${p.bg} !important`,
        color: `${p.fg} !important`,
      },
      ".cm-scroller":         { backgroundColor: `${p.bg} !important` },
      ".cm-gutters":          { backgroundColor: `${p.gutterBg} !important`,
                                color: p.gutterFg,
...
// ── Pro-writer base theme (typography, sizing — colour overridden by bgCompartment) ──
